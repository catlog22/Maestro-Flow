...
describe('E2E: Issue lifecycle — create -> update -> analyze -> solve -> close -> delete', () => {
  ...
  it('appends supplement and returns updated issue', async () => {
    const issue = await createIssue({ title: 'Supplement test', description: 'Desc' });
    expect(issue.supplements).toBeUndefined();

    const res = await app.request(`/api/issues/${issue.id}/supplements`, {
      method: 'PATCH',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ content: 'Extra context here', stage: 'post_creation', author: 'user' }),
    });
    expect(res.status).toBe(200);
    const updated = (await res.json()) as Issue;
    expect(updated.supplements).toHaveLength(1);
    expect(updated.supplements![0].content).toBe('Extra context here');
    expect(updated.supplements![0].stage).toBe('post_creation');
    expect(updated.supplements![0].author).toBe('user');
    expect(updated.supplements![0].created_at).toBeDefined();
  });

  it('appends multiple supplements (append-only)', async () => {
    const issue = await createIssue({ title: 'Multi supplement', description: 'Desc' });

    await app.request(`/api/issues/${issue.id}/supplements`, {
      method: 'PATCH',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ content: 'First', stage: 'post_creation', author: 'user' }),
    });
    const res2 = await app.request(`/api/issues/${issue.id}/supplements`, {
      method: 'PATCH',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ content: 'Second', stage: 'analysis', author: 'gemini' }),
    });
    expect(res2.status).toBe(200);
    const updated = (await res2.json()) as Issue;
    expect(updated.supplements).toHaveLength(2);
    expect(updated.supplements![0].content).toBe('First');
    expect(updated.supplements![1].content).toBe('Second');
    expect(updated.supplements![1].stage).toBe('analysis');
  });

  it('supplement on non-existent issue returns 404', async () => {
    const res = await app.request('/api/issues/NON-EXISTENT-ID/supplements', {
      method: 'PATCH',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ content: 'test', stage: 'general', author: 'user' }),
    });
    expect(res.status).toBe(404);
  });

  it('supplement validation rejects empty content', async () => {
    const issue = await createIssue({ title: 'Validate', description: 'Desc' });
    const res = await app.request(`/api/issues/${issue.id}/supplements`, {
      method: 'PATCH',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ content: '', stage: 'general', author: 'user' }),
    });
    expect(res.status).toBe(400);
  });

  it('supplement validation rejects invalid stage', async () => {
    const issue = await createIssue({ title: 'Validate', description: 'Desc' });
    const res = await app.request(`/api/issues/${issue.id}/supplements`, {
      method: 'PATCH',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ content: 'test', stage: 'invalid_stage', author: 'user' }),
    });
    expect(res.status).toBe(400);
  });

  it('supplement validation rejects missing author', async () => {
    const issue = await createIssue({ title: 'Validate', description: 'Desc' });
    const res = await app.request(`/api/issues/${issue.id}/supplements`, {
      method: 'PATCH',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ content: 'test', stage: 'general' }),
    });
    expect(res.status).toBe(400);
  });

  it('supplements persist across GET requests', async () => {
    const issue = await createIssue({ title: 'Persist', description: 'Desc' });
    await app.request(`/api/issues/${issue.id}/supplements`, {
      method: 'PATCH',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ content: 'Persisted note', stage: 'post_creation', author: 'user' }),
    });

    const fetched = await getIssue(issue.id);
    expect(fetched.supplements).toHaveLength(1);
    expect(fetched.supplements![0].content).toBe('Persisted note');
  });
});
