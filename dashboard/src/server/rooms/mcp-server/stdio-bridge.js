...
// Environment
// ---------------------------------------------------------------------------
process.stderr.write('[meeting-room-bridge] Starting up...\n');
...
process.stderr.write(`[meeting-room-bridge] PORT=${PORT} AGENT_ID=${AGENT_ID} TOKEN=${TOKEN ? 'set' : 'missing'}\n`);
if (!PORT || !TOKEN || !AGENT_ID) {
    process.stderr.write('Missing required env: MEETING_ROOM_MCP_PORT, MEETING_ROOM_MCP_TOKEN, MEETING_ROOM_AGENT_ID\n');
    process.exit(1);
}
...
async function handleRequest(req) {
    ...
    switch (req.method) {
        case 'initialize': {
            process.stderr.write(`[meeting-room-bridge] Received initialize request (id=${id})\n`);
        ...
        case 'tools/list': {
            process.stderr.write(`[meeting-room-bridge] Received tools/list (id=${id})\n`);
            sendResponse(makeResult(id, {
...
function startStdioTransport() {
    let buffer = '';
    process.stderr.write('[meeting-room-bridge] Transport started, waiting for stdin...\n');
