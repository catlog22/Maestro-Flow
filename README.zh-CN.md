...
*我不写代码 —— Claude Code 和 Codex 写。但告诉它们做什么、按什么顺序、带什么上下文、然后验证结果 —— 这才是耗时间的地方。Maestro 把这些编排自动化了。*
...
Maestro 保留了验证过的核心思路，用全新的理念重建：**更少仪式感，更快执行。** 规格驱动的阶段管线设计受 [GET SHIT DONE (GSD)](https://github.com/gsd-build/get-shit-done) 启发 —— 它的上下文工程方法和原子提交纪律确实优雅。Maestro 采纳了这些设计模式，同时补上 GSD 没有的东西：实时可视化看板、基于 Claude Agent SDK 的多智能体执行、以及自主运行的 Commander 让管线持续推进。

**相比 CCW 改了什么：**
- 砍掉了重型的 session/beat 编排层 —— 替换为轻量的 skill 路由
- 终端仪表盘升级为 Linear 风格的 Web 看板 UI
- 统一所有 CLI 工具调用到 `ccw cli` 单一接口
...
你描述你想要什么。Maestro 决定用哪些智能体、按什么顺序、带什么上下文 —— 然后驱动它完成。

```bash
# 自然语言 —— Maestro 路由到最优命令链
...
| `/maestro "..."` | AI 路由命令链 | 描述意图，让 Maestro 决定 |
...
- **CLI 适配器** — Codex、Gemini、Qwen、OpenCode 全部通过 `ccw cli` 调用
...
将 Maestro 工具暴露给 Claude Desktop 和其他 MCP 客户端：
...
| 文档 | 说明 |
|------|------|
| [命令使用指南](docs/command-usage-guide.md) | 全部 36 个命令，含工作流图表 |
| [Issue-看板集成](docs/issue-kanban-integration.md) | 数据模型、生命周期、API、WebSocket、Commander |
| [融合架构设计](docs/fusion-design.md) | 系统设计（目录结构、Schema、架构原则） |
| [GSD 架构参考](docs/gsd-architecture-reference.md) | 原始 GSD 架构参考 |
...
- **[GET SHIT DONE](https://github.com/gsd-build/get-shit-done)** by TACHES — 规格驱动开发模型、上下文工程理念、原子提交纪律，塑造了 Maestro 的管线设计。GSD 证明了结构化的 meta-prompting 才是大规模驱动 AI 智能体的正确方式。

- **[Claude-Code-Workflow](https://github.com/catlog22/Claude-Code-Workflow)** — Maestro 的前身。CCW 开创了多 CLI 编排（Gemini + Codex + Qwen + Claude）、skill 路由工作流、team agent 架构。Maestro 是 CCW 从零重建 —— 更快、更精简，加上可视化看板和自主 Commander。
